...
#         logger.info("✅ RuleEngine - Using Neon PostgreSQL")
...
#             for rule in rules:
#                 if rule.get('trigger_category_id') == category_id:
...
#                         if rec_id in category_map:
#                             recommended_names.append(category_map[rec_id])
                    
#                     return recommended_names if recommended_names else ['Healthy Cafe', 'Fresh Juice', 'Salad Restaurant']
        
#         except Exception as e:
#             logger.error(f"Error getting recommendations from Neon: {e}")
...
#                 # Get category from Neon
#                 category = neon_db_service.get_category_by_id(category_id)
#                 category_name = category.get("name", category_id) if category else category_id
                
#                 # Determine if unhealthy
#                 is_unhealthy = category.get("is_unhealthy", False) if category else category_id in ['fast_food', 'bar_pub']
