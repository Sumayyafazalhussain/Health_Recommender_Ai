...
#     logger.error(f"Failed to import ImageAnalysisService: {e}")
#     image_service = None
...
# ):
#     """Full image analysis with more details"""
#     try:
#         if not image_service:
#             raise HTTPException(status_code=500, detail="Image service not available")
...
#         if include_details and result.get("status") == "analysis_complete":
#             detected_items = result.get("detected_items", [])
#             result["summary"] = {
#                 "total_items": len(detected_items),
#                 "healthy_count": sum(1 for item in detected_items if item.get("is_healthy")),
#                 "unhealthy_count": sum(1 for item in detected_items if not item.get("is_healthy", True)),
#                 "avg_health_score": sum(item.get("health_score", 5) for item in detected_items) / len(detected_items) if detected_items else 0
#             }
        
#         return result
        
#     except Exception as e:
#         logger.error(f"Full image analysis error: {e}")
#         raise HTTPException(status_code=500, detail=str(e))
# from fastapi import APIRouter, HTTPException, Query
# from typing import Optional
# import logging
...
#                             raise ValueError("Invalid image format. Please provide a valid JPEG, PNG, or GIF image.")
#                     else:
...
#                 "test_url": f"{base_url}/api/image/analyze-quick?image_url=https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=400"
#             },
#             {
#                 "name": "Salad Test",
#                 "url": "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400",
#                 "test_url": f"{base_url}/api/image/analyze-quick?image_url=https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400"
#             },
#             {
#                 "name": "Base64 Test (Small)",
#                 "url": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==",
#                 "test_url": f"{base_url}/api/image/analyze-quick?image_url=data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
#             }
#         ]
#     }
...
#             {
#                 "name": "Fruit Bowl",
#                 "url": "https://images.unsplash.com/photo-1567306301408-9b74779a11af?w=400",
...
#     """
#     try:
#         logger.info(f"Quick analysis requested for image: {image_url[:50]}...")
...
#         # Simple food detection (you can replace this with AI/ML later)
#         img_lower = str(image_url).lower()
        
#         # Check for food types
#         detected_foods = []
#         if any(keyword in img_lower for keyword in ['burger', 'hamburger', 'cheeseburger']):
#             detected_foods.append("burger")
#         if any(keyword in img_lower for keyword in ['pizza', 'pizzeria']):
#             detected_foods.append("pizza")
#         if any(keyword in img_lower for keyword in ['salad', 'vegetable', 'veggie']):
#             detected_foods.append("salad")
#         if any(keyword in img_lower for keyword in ['fruit', 'apple', 'banana']):
#             detected_foods.append("fruit")
            
#         if not detected_foods:
#             detected_foods = ["food"]  # Default
            
#         # Health assessment
#         unhealthy_keywords = ['burger', 'pizza', 'fried', 'fast food', 'cheeseburger']
#         healthy_keywords = ['salad', 'fruit', 'vegetable', 'healthy', 'organic']
...
#                     "height": height,
#                     "format": format,
#                     "size_kb": round(len(image_data.getvalue()) / 1024, 2)
#                 },
#                 "food_detection": {
...
#                 },
#                 "estimated_nutrition": {
#                     "calories": 450 if is_unhealthy else 320,
#                     "protein_g": 25 if is_unhealthy else 15,
#                     "carbs_g": 40 if is_unhealthy else 45,
#                     "fat_g": 25 if is_unhealthy else 10
#                 }
#             },
#             "recommendations": recommendations,
#             "alternatives": location_alternatives if location_alternatives else [
#                 {"name": "General healthy alternative", "type": "suggestion"}
#             ],
#             "location_used": lat is not None and lng is not None,
...
#                 "status": "error",
#                 "message": str(e),
#                 "tip": "Please check the image URL or try a different image",
#                 "example_urls": [
...
#                 "tip": "Try again with a simpler image or check the logs"
#             }
#         )
...
#     """
#     try:
#         logger.info(f"Full analysis requested for image: {image_url[:50]}...")
...
#         # Mock detailed analysis (you can integrate AI here)
#         detailed_analysis = {}
#         if include_details:
#             detailed_analysis = {
#                 "color_analysis": {
#                     "dominant_colors": ["#FFA726", "#FF7043", "#8D6E63"],
#                     "brightness_score": 65,
#                     "contrast_score": 42
#                 },
#                 "composition": {
...
#                     "focus_score": 78
#                 },
#                 "ingredient_estimation": [
#                     {"name": "bread", "confidence": 85},
#                     {"name": "meat", "confidence": 75},
#                     {"name": "cheese", "confidence": 65},
#                     {"name": "vegetables", "confidence": 45}
#                 ]
#             }
...
#                         "calories_saved": 180,
#                         "health_improvement": "+45 points",
#                         "protein_comparison": "similar"
#                     },
#                     {
#                         "name": "Vegetable Wrap with Hummus",
#                         "calories_saved": 220,
#                         "health_improvement": "+50 points",
#                         "protein_comparison": "slightly lower"
#                     }
#                 ],
#                 "details": detailed_analysis if include_details else None
#             },
#             "location_context": {
#                 "used": lat is not None and lng is not None,
#                 "nearby_healthy_options": 3 if lat and lng else 0
#             },
#             "message": "Full image analysis completed"
#         }
...
#                 "status": "error",
#                 "message": str(e),
#                 "tip": "Ensure the image URL is valid and accessible"
#             }
#         )

# @router.get("/analyze-menu")
# async def analyze_menu_image(
#     image_url: str = Query(..., description="URL of menu image"),
#     lat: Optional[float] = Query(None, description="Your latitude"),
#     lng: Optional[float] = Query(None, description="Your longitude")
# ):
#     """
#     Analyze restaurant menu images
#     """
#     try:
#         # Download image
#         image_data = await download_image(image_url)
        
#         # Mock menu analysis (you can integrate OCR/AI here)
#         return {
#             "status": "success",
#             "menu_analysis": {
#                 "detected_items": [
#                     {"name": "Cheeseburger", "price": "$12.99", "health_score": 25},
#                     {"name": "Caesar Salad", "price": "$10.99", "health_score": 75},
#                     {"name": "Grilled Chicken", "price": "$14.99", "health_score": 65},
#                     {"name": "French Fries", "price": "$5.99", "health_score": 20}
#                 ],
#                 "healthiest_option": {
#                     "name": "Caesar Salad",
#                     "health_score": 75,
#                     "reason": "High in vegetables, lower in calories"
#                 },
#                 "unhealthiest_option": {
#                     "name": "Cheeseburger with Fries",
#                     "health_score": 15,
#                     "reason": "High in saturated fat and calories"
#                 },
#                 "price_range": {"low": "$5.99", "high": "$14.99", "average": "$10.99"}
#             },
#             "recommendations": [
#                 "Choose grilled over fried options",
#                 "Ask for dressings on the side",
#                 "Select vegetable sides instead of fries"
#             ],
#             "location_alternatives": [
#                 {
#                     "name": "Healthy Bistro",
...
#                 "status": "error",
#                 "message": str(e),
#                 "tip": "Make sure the menu image is clear and readable"
#             }
#         )
